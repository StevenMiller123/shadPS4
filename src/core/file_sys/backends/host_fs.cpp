...
bool HostFile::Map(VAddr addr, u64 size, u64 offset, MemoryMapFlags flags, MemoryProt raw_prot) {
    if (!IsOpen()) {
        return false;
    }
    ...
    address_space.MapFile(addr, size, offset, flags, static_cast<u32>(raw_prot),
                          m_file.GetFileMapping());
    return true;
}
